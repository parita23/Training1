...
data.save(function(err) {
    data.save(function(err) {
        if (err) {
            ...
            fileobject.mv("public/userphoto/" + filename, function(err) {
                if (err) throw err;
                // res.send("File Uploaded");
                res.render('admin/users/add', { msg: "Record stored successfully" });
         });
        }
    })
});
...
router.get('/display', function(req, res, next) {
    UserModel.find(function(err, data) {
        if (err) {
            ...
            res.render('admin/users/display', { mydata: data });
        }
    }).lean();
});
...
router.get('/edit/:id', function(req, res, next) {
    ...
    UserModel.findById(editid, function(err, data) {
        if (err) {
            ...
            res.render('admin/users/edit', { mydata: data })
        }
    }).lean();
});
