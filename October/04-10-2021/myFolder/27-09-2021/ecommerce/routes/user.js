      ...
      if (db_email == null) {
        ...
        async function main() {
          ...
          let mailOptions = {
            ...
            html: "<h2>Please find your forgot password link </h2> <br>http://127.0.0.1:3000/user/change-password" // html body
          };
...
router.post('/signupprocess', function(req, res, next) {
    UserModel.find({ "email": req.body.email}, function (err, user_array) {
        if (err){ 
          console.log(err)
          throw err
        }
  ...
  router.get('/userDisplay', function(req, res, next) {
    UserModel.find(function(err,data){
        if(err) throw err;
        res.render('user/displayUser' , {mydata : data})
    }).lean();
  });
...
router.get('/delete/:id', function(req, res, next) {
  ...
  router.get('/edit/:id', function(req, res, next) {
    ...
    UserModel.findById(id,function(err,data){
      if(err){
        ...
        console.log(data)
        res.render('user/editUser' ,{mydata : data})
      }
    }).lean();
  });
  ...
  router.post('/change-password', function (req, res, next) {
    ...
    UserModel.findOne({ "email": myemail }, function (err, db_users_array) {
        ...
        if (opass == db_users_array.password) {
          ...
          if (opass == npass) {
            ...
            if (npass == cpass) {
              ...
              UserModel.findOneAndUpdate({ "email": myemail }, { $set: { "password": npass } }, function (err) {
  ...
  router.get('/home', function (req, res, next) {
    ...
    res.render('user/userhome', { myemail: myemail });
  });
  ...
  router.post('/edit/:id', function(req, res, next) {
    ...
    UserModel.findByIdAndUpdate(req.params.id,inputData,function(err){
      if(err){
        console.log("Error from update section " + err)
      }else{
        console.log(req.body);
